...
openssl aes-256-cbc -K ${!ssh_key_var_name} -iv ${!ssh_iv_var_name} -in deploy/build\+ts-api@travis-ci.org.enc -out /tmp/build\+ts-api@travis-ci.org -d
eval "$(ssh-agent -s)"
...
openssl aes-256-cbc -K ${!vault_key_var_name} -iv ${!vault_iv_var_name} -in deploy/vault_pass.txt.enc -out /tmp/vault_pass.txt -d
