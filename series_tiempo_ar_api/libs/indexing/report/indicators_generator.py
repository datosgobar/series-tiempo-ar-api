...
class IndicatorsGenerator(object):
    ...
    def calculate_series_indicators(self, node, data_json):
        ...
        self.task.indicator_set.create(type=Indicator.FIELD_NOT_INDEXABLE, value=not_indexable,
                                       node=node)
        updated = self.task.indicator_set.get_or_create(type=Indicator.FIELD_UPDATED, node=node)[
            0].value

        not_updated = indexable - updated
        self.task.indicator_set.create(type=Indicator.FIELD_NOT_UPDATED, value=not_updated,
                                       node=node)
    ...
    def calculate_distribution_indicators(self, node, data_json):
        ...
        self.task.indicator_set.create(type=Indicator.DISTRIBUTION_INDEXABLE, value=indexable,
                                       node=node)
        not_indexable = Distribution.objects.filter(dataset__catalog=catalog,
                                                    dataset__indexable=False,
                                                    dataset__present=True).count()
        self.task.indicator_set.create(type=Indicator.DISTRIBUTION_NOT_INDEXABLE,
                                       value=not_indexable, node=node)

        updated = self.task.indicator_set.get_or_create(type=Indicator.DISTRIBUTION_UPDATED,
                                                        node=node)[0].value

        not_updated = indexable - updated
        self.task.indicator_set.create(type=Indicator.DISTRIBUTION_NOT_UPDATED, value=not_updated,
                                       node=node)
    ...
    def calculate_dataset_indicators(self, node, data_json):
        ...
        self.task.indicator_set.create(type=Indicator.DATASET_NOT_INDEXABLE, value=not_indexable,
                                       node=node)
        ...
        not_updated = indexable - updated
        self.task.indicator_set.create(type=Indicator.DATASET_NOT_UPDATED, value=not_updated,
                                       node=node)
