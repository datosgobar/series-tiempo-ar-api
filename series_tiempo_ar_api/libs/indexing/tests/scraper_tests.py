...
@mock.patch('series_tiempo_ar_api.libs.indexing.distribution_validator.DistributionRepository.get_time_index_series',
            return_value=get_mock_index_series())
class DistributionValidatorTests(TestCase):
    def setUp(self):
        self.task = ReadDataJsonTask()
        self.task.save()
        self.validator = DistributionValidator(read_local=True)

    def test_validator(self, *_):
        ...
        result = self.validator.run(distribution, catalog)
    ...
    def test_missing_dataframe_column(self, *_):
        ...
        distribution = MockDistribution(distribution)
        self.validator.run(distribution, catalog)
